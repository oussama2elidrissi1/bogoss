            ...
            <form method="POST" action="{{ route('admin.bookings.update', $booking) }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="service_id" class="input-field" required>
                        @foreach($services as $service)
                            <option value="{{ $service->id }}" data-price="{{ $service->price }}" {{ old('service_id', $booking->service_id) == $service->id ? 'selected' : '' }}>{{ $service->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="staff_id" class="input-field">
                        ...
                        @foreach($staff as $member)
                            <option value="{{ $member->id }}" data-payouts='@json($member->services->pluck("pivot.payout_percentage","id"))' {{ old('staff_id', $booking->staff_id) == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="date" class="input-field" value="{{ old('date', $booking->date->toDateString()) }}" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Time</label>
                        <input type="time" name="time" class="input-field" value="{{ old('time', $booking->time) }}" required>
                    </div>
                </div>
                <div>
                ...
                <div>
                    ...
                    <textarea name="notes" class="input-field" rows="3">{{ old('notes', $booking->notes) }}</textarea>
                </div>
