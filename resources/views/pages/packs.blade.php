...
@section('title', 'Réserver vos services')
        ...
        <div class="glass-card p-4 bg-green-50 border-l-4 border-green-500">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-sm text-green-700">Référence: {{ session('booking_reference') }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8" id="pack-builder">
            <div class="lg:col-span-2">
                ...
                <div class="glass-card p-6 mb-6">
                    ...
                    <!-- Search Bar -->
                    <form method="GET" action="{{ route('packs') }}" class="mb-6">
                        <div class="relative">
                            <input
                                type="text"
                                name="search"
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                        ...
                        @endphp
                        <a href="{{ route('packs', ['category' => 'all', 'search' => request('search')]) }}" 
                           class="relative overflow-hidden rounded-xl p-4 transition-all duration-300 transform hover:scale-105 {{ $isAllActive ? 'ring-2 ring-primary shadow-lg' : 'hover:shadow-md' }}">
                            <div class="absolute inset-0 bg-gradient-to-br {{ $catAll['color'] }} opacity-{{ $isAllActive ? '100' : '20' }}"></div>
                            <div class="relative flex flex-col items-center text-center">
                                <span class="text-3xl mb-2">{{ $catAll['icon'] }}</span>
                                <span class="text-sm font-semibold {{ $isAllActive ? 'text-white' : 'text-gray-900' }}">{{ $catAll['label'] }}</span>
                                @if($isAllActive)
                        ...
                        @foreach($categories as $category)
                            ...
                            @endphp
                            <a href="{{ route('packs', ['category' => $category, 'search' => request('search')]) }}" 
                               class="relative overflow-hidden rounded-xl p-4 transition-all duration-300 transform hover:scale-105 {{ $isActive ? 'ring-2 ring-primary shadow-lg' : 'hover:shadow-md' }}">
                                <div class="absolute inset-0 bg-gradient-to-br {{ $catInfo['color'] }} opacity-{{ $isActive ? '100' : '20' }}"></div>
                                <div class="relative flex flex-col items-center text-center">
                                    <span class="text-3xl mb-2">{{ $catInfo['icon'] }}</span>
                                    <span class="text-sm font-semibold {{ $isActive ? 'text-white' : 'text-gray-900' }}">{{ $catInfo['label'] }}</span>
                                    @if($isActive)
                                        <span class="absolute top-0 right-0 w-6 h-6 bg-white rounded-full flex items-center justify-center">
                                            <span class="text-xs">✓</span>
                                        </span>
                    ...
                    @if(request()->filled('search') || (request()->filled('category') && request('category') !== 'all'))
                        <div class="mt-4 flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                            <span class="text-sm text-gray-700">
                                <strong>{{ $services->count() }}</strong> service(s) trouvé(s)
                                @if(request()->filled('search'))
                                    pour "<strong>{{ $searchQuery }}</strong>"
                                @endif
                            </span>
                            <a href="{{ route('packs') }}" class="text-sm text-primary hover:text-primary-dark font-medium">
                                ✕ Réinitialiser
                            </a>
                        </div>
                    @endif
                            ...
                            >
                                ...
                                <div class="p-4">
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="min-w-0">
                                            <h3 class="font-serif text-lg font-bold text-gray-900 mb-1">{{ $service->name }}</h3>
                ...
                @else
                    <div class="glass-card p-8 text-center">
                        ...
                        <a href="{{ route('packs') }}" class="btn-primary inline-block">
                    ...
                    <div id="booking-form-section" class="hidden border-t border-gray-100 mt-5 pt-5">
                        ...
                        <form id="booking-form" class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="date" id="booking-date" class="input-field" min="{{ now()->toDateString() }}" value="{{ now()->toDateString() }}" required>
                            </div>
                            <div>
...
<script>
  (function () {
    ...
    const promotions = @json($promoData);
