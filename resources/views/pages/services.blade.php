...
<div class="min-h-screen bg-gradient-to-b from-white to-gray-50">
    <section class="gradient-wellness py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center text-white">
                <h1 class="font-serif text-5xl font-bold mb-4">{{ __('pages.services.hero_title') }}</h1>
                <p class="text-xl max-w-2xl mx-auto">{{ __('pages.services.hero_subtitle') }}</p>
            </div>
        </div>
    </section>
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex flex-wrap gap-3 mb-8">
            <a href="{{ route('services', ['category' => 'All', 'q' => $searchQuery]) }}" class="px-6 py-2 rounded-full font-medium transition-all duration-300 {{ $selectedCategory === 'All' ? 'bg-primary text-white shadow-lg scale-105' : 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200' }}">{{ __('pages.common.all') }}</a>
                    ...
                    <div class="glass-card overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <a href="{{ route('booking', ['service_id' => $service->id]) }}" class="w-full btn-primary inline-block text-center {{ $service->available ? '' : 'opacity-50 pointer-events-none' }}">
                                {{ $service->available ? __('pages.common.book') : __('pages.services.unavailable') }}
                            </a>
                        </div>
                    </div>
                @endforeach
        ...
        @else
            <div class="text-center py-16">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">{{ __('pages.services.none_title') }}</h3>
                <p class="text-gray-600">{{ __('pages.services.none_subtitle') }}</p>
            </div>
        @endif
    </section>
</div>
